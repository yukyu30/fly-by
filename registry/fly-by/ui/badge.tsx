 ...
 * fly-by Badge
 *
 * クラス構造: badge -appearance-{value} -color-{value} -size-{value}
 *
 * @example
 * <Badge color="informative">New</Badge>
 * <Badge appearance="solid" color="positive">Success</Badge>
 */
...
function Badge({
  ...
  return (
    <span
      data-slot="badge"
      className={cn(badgeVariants({ appearance, color, size, className }))}
      {...props}
    />
  )
}
