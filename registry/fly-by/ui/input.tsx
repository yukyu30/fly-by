 ...
 * fly-by Input
 *
 * クラス構造: input -appearance-{outlined|filled} -size-{s|m|l}
 */

const inputVariants = cva(
  "input flex w-full rounded-md transition-all outline-none disabled:cursor-not-allowed disabled:opacity-50",
  {
    variants: {
      appearance: {
...
function Input({ className, appearance, size, type, ...props }: InputProps) {
  return (
    <input
      type={type}
      data-slot="input"
      className={cn(inputVariants({ appearance, size, className }))}
      {...props}
    />
  )
}
