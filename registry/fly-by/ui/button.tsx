 ...
 * fly-by Button
 *
 * クラス構造: button -appearance-{value} -color-{value} -size-{value}
 *
 * @example
...
const buttonVariants = cva(
  // Base styles
  "button inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-offset-2",
  {
    variants: {
      appearance: {
...
function Button({
  ...
  return (
    <Comp
      data-slot="button"
      className={cn(buttonVariants({ appearance, color, size, width, className }), iconOnlyClass)}
      {...props}
    >
      {leading}
      {children}
